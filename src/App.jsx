...
import { AlertCircle, Send, Mail, MessageSquare, Phone, ShieldCheck, Loader2, Copy, Check, Info, Building2, GraduationCap } from 'lucide-react';
...
function App() {
  ...
  const [senderLevel, setSenderLevel] = useState('School'); // NEW STATE FOR TOGGLE
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // The prompt now dynamically changes based on the toggle!
      const prompt = `
        ...
        AUDIENCE: Parents and Community (8th-grade reading level).
        
        FACTS: ${input}

        INSTRUCTIONS:
        1. Emphasize safety and student well-being first.
        2. Keep the tone reassuring but factual. 
        ...
        Respond with a JSON object using this exact structure:
        {
          ...
          "sms": "[Max 160 chars] [${senderLevel} Name]: [Brief update]. [Action item].",
          "script": "[3-sentence script for front office staff answering phones]"
        }
      `;
      ...
      <nav className="bg-white border-b border-slate-200 px-8 py-5 flex justify-between items-center shadow-sm sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">District Operations</p>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto p-6 lg:p-10 grid lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
            ...
            <textarea 
              className="w-full h-48 p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm leading-relaxed focus:ring-4 focus:ring-blue-100 focus:border-blue-500 focus:outline-none transition-all resize-none shadow-inner"
            ...
            >
              ...
              {loading ? "Drafting Protocols..." : "Generate Alerts"}
            </button>
          </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          {!results && !loading && (
            <div className="h-full min-h-[500px] flex flex-col items-center justify-center border-4 border-dashed border-slate-200 rounded-3xl bg-white/50 text-slate-400">
              ...
              <p className="font-semibold">Ready for Incident Details</p>
              <p className="text-sm mt-1 text-slate-400">Output will be formatted for Email, SMS, and Voice.</p>
            </div>
          )}
          ...
          {loading && (
            <div className="h-full min-h-[500px] flex flex-col items-center justify-center rounded-3xl bg-white border border-slate-100 shadow-xl">
              ...
              <p className="text-lg font-bold text-slate-800">Processing Response</p>
              <p className="text-slate-500 animate-pulse">Consulting communication standards...</p>
            </div>
          )}
            ...
            <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden shadow-xl shadow-slate-200/50">
                <div className="bg-slate-50 px-8 py-5 border-b border-slate-200 flex justify-between items-center">
                  <div className="flex items-center gap-2 font-bold text-slate-700 uppercase tracking-wider text-xs">
                    <Mail size={16} className="text-blue-600" /> Email Notification
                  </div>
                  <button 
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                  <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                    <div className="flex items-center gap-2 font-bold text-slate-700 uppercase tracking-wider text-[10px]">
                      <MessageSquare size={14} className="text-green-600" /> SMS Alert
                    </div>
                    <button 
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                  <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                    <div className="flex items-center gap-2 font-bold text-slate-700 uppercase tracking-wider text-[10px]">
                      <Phone size={14} className="text-purple-600" /> Staff Script
                    </div>
                    <button 
